  ...
  <body>
   <div class="container">
        ...
        <nav class="nav d-flex justify-content-between fixed-top " style="background-color: {{$navbar->top_navbar_bg_color}};">       
                                 <!-- For mobile tablets -->    
                                <a class=" p-2 bottom-nav" id="first-nav" href="{{asset(route('public.welcome'))}}" style="color:{{$navbar->top_text_color}};"><img src="{{asset($sitesettings->logo)}}" alt="Logo" height="25px"></a>
                            
                               <div class=" p-2 ">
                                <a href="{{asset(route('public.welcome'))}}"><h5 id="site-title" style="color:{{$navbar->top_text_color}};">{{$sitesettings->name}}</h5></a>
                               </div> 
                                  ...
                                  <div class="p-2 bottom-nav desknav" >
                                    ...
                                    <a  href="{{ route('logout') }}"
                                       onclick="event.preventDefault();
                                                     document.getElementById('logout-form').submit();" style="color:{{$navbar->top_text_color}};">
                                          {{ __('Logout') }}
                                    </a>
                                    </div>
      ...
      <nav class="nav d-flex justify-content-between fixed-bottom " style="background-color: {{$navbar->bottom_navbar_bg_color}};">
                        ...
                        @else
                                  ...
                                  <div class="p-2 bottom-nav smallnav">
                                    ...
                                    <a  href="{{ route('logout') }}"
                                       onclick="event.preventDefault();
                                                     document.getElementById('logout-form').submit();" style="color:{{$navbar->bottom_text_color}};">
                                          {{ __('Logout') }}
                                    </a>
                                    </div>
                                    <form id="logout-form" action="{{ route('logout') }}" method="POST" style="display: none;">
